                            ...
                            <div class="shop-sidebar-widget">
                                ...
                                <form action="{{ route('filter.bySpecifications') }}" method="GET">
                                    <ul class="shop-sidebar-widget-list shop-sidebar-widget-list-size">
                                        <ul>
                                            <li>
                                                <input type="checkbox" id="spec_small" name="specifications[]"
                                                    value="small"
                                                    {{ in_array('small', request()->input('specifications', [])) ? 'checked' : '' }} />
                                                <label for="spec_small">Small ({{ $smallCount }})</label>
                                            </li>
                                            <li>
                                                <input type="checkbox" id="spec_large" name="specifications[]"
                                                    value="large"
                            ...
                            <div class="shop-toolbar">
                                <div class="shop-toolbar-left-side d-flex">
                                    ...
                                    <p class="shop-products-result-count">
                                        Showing {{ $totalProducts->firstItem() }}–{{ $totalProducts->lastItem() }} of
                                        results
                                    </p>
                                        ...
                                        @if ($product && $product->product)
                                            <div class="col-md-6 col-lg-4 mb-5">
                                                ...
                                                <div class="card position-relative box-shad h-100 mb-4">
                                                    <div class="p-2">
                                                        @if ($product->color && $product->color->count() > 0)
                                                            <a href="{{ route('single.product', ['slug' => $product->product->slug]) }}"
                                                                class="card_height">
                                                                @php
                                                                    $imagePath =
                                                                        'storage/' .
                                                                        $product->color->first()->single_image;
                                                                @endphp
                                                    ...
                                                    <div class="card-body">
                                                        <h6 class="card-title">
                                                            {{ $product->product ? $product->product->title : 'Product Title Not Available' }}
                                                        </h6>
                                                        <span>{{ $product->color ? $product->color->count() : 0 }}
                                                            colors</span>
                                                        ...
                                                        <div
                                                            ...
                                                            <form action="{{ route('buy.now', $product->id) }}"
                                                                ...
                                                                <input type="hidden" name="product_color_id"
                                                                    value="{{ $product->color ? $product->color->id : '' }}">
                                                                ...
                                                                @if ($product)
                                                                    @if (is_null($product->offer_price))
                                                                        <span class="product-card-regular-price fw-600">
                                                                            {{ $currencySymbol }}{{ number_format($product->offer_price * $exchangeRate, 2) }}
                                                                        </span>
                                                                    @else
                                                                        <span class="product-card-old-price">
                                                                            <del>{{ $currencySymbol }}{{ number_format($product->price * $exchangeRate, 2) }}</del>
                                                                        </span>
                                                                        <span class="product-card-regular-price fw-600">
                                                                            {{ $currencySymbol }}{{ number_format($product->offer_price * $exchangeRate, 2) }}
                                                                        </span>
                            ...
                            <div class="pagination-area">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        ...
                                        {!! $totalProducts->withQueryString()->links('pagination::bootstrap-5') !!}
                                        {{-- {{ $totalProducts->links() }} --}}
