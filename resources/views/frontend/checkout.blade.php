    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="name" name="name" placeholder="First name *" value="{{$billingAddress->name ?? ''}}">
            </div>
            @error('name')
        ...
        <div class="col-lg-6">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="email" id="email" name="email" placeholder="email *" value="{{$billingAddress->email ?? ''}}">
            </div>
            @error('email')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="mobile" name="mobile" placeholder="Phone *" value="{{$billingAddress->mobile ?? ''}}">
            </div>
            @error('mobile')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="country" name="country" placeholder="country *"value="{{$billingAddress->country ?? ''}}">
            </div>
            @error('country')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="state" name="state" placeholder="State *"value="{{$billingAddress->state ?? ''}}">
            </div>
            @error('state')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="city" name="city" placeholder="city *" value="{{$billingAddress->city ?? ''}}">
            </div>
            @error('city')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="address" name="address" placeholder="Address *" value="{{$billingAddress->address ?? ''}}">
            </div>
            @error('address')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="col-lg-12">
            <div class="checkout-form-row">
                <input class="checkout-form-input" type="text" id="pincode" name="pincode" placeholder="pincode *"value="{{$billingAddress->pincode ?? ''}}">
            </div>
            @error('pincode')
            <span class="text-danger">{{$message}}</span>
            @enderror
...
<!-- shipping Address Starts -->
    ...
    $shippingAddresses = Addresses::where('user_id', $user->id)
                                  ...
                                  ->get();
    $firstAddressId = $shippingAddresses->first() ? $shippingAddresses->first()->id : null;
@endphp
    ...
    <input type="hidden" name="selected_shipping_address_id" value="{{$shippingAddress->id}}">
    ...
    {{ $shippingAddress->id === $firstAddressId ? 'checked' : '' }}
/>
        <label class="form-check-label" for="flexRadioDefault{{$shippingAddress->id}}">
            <h5 class="fs-20 fw-500">
                {{$shippingAddress->name}}
            </h5>
        </label>
                    ...
                    <p class="fs-15 pl-1em">{{$shippingAddress->address}}</p>
                    <p class="fs-16 pl-1em">Contact -(+91) {{$shippingAddress->mobile}}</p>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="d-flex justify-content-between">
                   <a href="{{ route('edit.shipping.address', $shippingAddress->id) }}"> <span class="fs-16 fw-500 text-black">Edit</span></a>
                   <a href="{{ route('remove.shipping.address', $shippingAddress->id) }}">
...
@endforeach
              ...
              >
              <a href="{{route('add.shipping.address')}}">  <p class="fs-6 text-success px-5">Add Address</p></a>
...
<!-- tst ends -->
    ...
    <div class="ship-box-info"  id="shipping-address-form" style="display: none;">
        <div class="row">
            <div class="col-lg-6">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" name="shipping_name" placeholder="name *">

                </div>
            </div>
            <div class="col-lg-6">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="email" name="shipping_email" placeholder="email" >

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" name="shipping_mobile" placeholder="Phone">

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" name="shipping_country" placeholder="country" >

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" name="shipping_state" placeholder="state name" >

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" placeholder="city name" name="shipping_city">

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" placeholder="Address" name="shipping_address" >

                </div>
            </div>
            <div class="col-lg-12">
                <div class="checkout-form-row">
                    <input class="checkout-form-input" type="text" name="shipping_pincode" placeholder="Postcode *" >
    ...
    <table class="table">
        ...
        @foreach($cart as $item)
          <tr class="cart_item">
            ...
            <td class="product-total">
              {{ $currencySymbol }}<span class="amount">{{ number_format($totalAmount, 2) }}</span>
            </td>
          </tr>
        @endforeach
      </tbody>
      <tfoot>
        <tr class="cart-subtotal">
          <th>Cart Subtotal</th>
          <td>{{ $currencySymbol }}<span class="amount">{{ number_format($singleAmountCountryCurrency, 2) }}</span></td>
        ...
        <tr class="cart-subtotal">
          ...
          <td>{{ $currencySymbol }}<span class="amount">{{ $res_coupon }}</span></td>
        </tr>
        <tr class="order-total">
          <th>Order Total</th>
          <td><strong>{{ $currencySymbol }} <span class="amount">{{ number_format((float)$finalTotalAmount - (float)$res_coupon, 2) }}</span></strong></td>
        </tr>
      </tfoot>
    </table>
  </div>
  ...
  <div class="col-lg-12">
      <div class="checkout-form-row">
          ...
          <label>
              <input type="radio" name="payment_method" value="razorpay" id="razorpay-option"> Razorpay
          </label>
          <label>
              <input type="radio" name="payment_method" value="cod" id="cod-option"> Cash on Delivery
          </label>
      </div>
  ...
  <div class="text-center border-top-1 pt-4">
      <button type="button" class="btn btn-primary btn-full btn-md" onclick="validateForm()">Place Order</button>
  </div>
...
<!-- testing end -->
<script>
    var stripe = Stripe('pk_test_51P8d5PSGxcIrVgVcJoxGtRjnHbUTsAbYQEUn6McgRqIqPaaQkaIhBx5ecPc1IDhuo7IIYG49cC4J5zeSxnf7XCay00B3fbQZJq'); // Replace with your Stripe publishable key
