        ...
        @foreach ($sessionCart as $sessionItem)
            @php
                if (is_array($sessionItem)) {
                    ...
                    $itemPriceInCountryCurrency = 0;
                    if ($product) {
                        // Get the lowest offer price for the product color
                        $offerPrice = \App\Models\ProductColor::where('product_id', $sessionItem['product_id'])->min('offer_price');
                        if (!$offerPrice) {
                            $offerPrice = $product->offer_price;
                        }
                        $itemPriceInCountryCurrency = $offerPrice * $exchangeRate;
            ...
            <div class="cart-product-item">
                <a href="product-details.html" class="cart-product-thum">
                    <img src="{{ asset('storage/' . $sessionItem['image']) }}" alt="Product Cart One" />
                </a>
                <div class="cart-product-content">
                    <h6 class="cart-product-content-title">
                        <a href="">{{ $sessionItem['product_title'] }}</a>
                    </h6>
                    <div class="cart-product-content-bottom">
                        <span class="cart-product-content-quantity">{{ $sessionItem['quantity'] }} ×</span>
                        <span class="cart-product-content-amount">
                            <bdi>
                                <span class="visually-hidden">Price:</span>
                                {{ $currencySymbol }}<span class="item-total-price">{{ number_format($itemPriceInCountryCurrency, 2) }}</span>
                ...
                <button type="button"
                        ...
                        data-product-id="{{ $sessionItem['product_id'] }}">Remove</button>
            </div>
        @endforeach
    @endif
@endauth            </div>
            <div class="offcanvas-cart-footer">
                <div class="mini-cart-total">
                    ...
                    <span class="mini-cart-amount">
                        <bdi>
                            <span class="currency-symbol">{{ $currencySymbol }}</span>{{ number_format($totalAmount * $exchangeRate, 2) }}
                        </bdi>
                    </span>
                </div>
