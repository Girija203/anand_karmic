...
@section('content')
                                                        ...
                                                        <td class="name">
                                                            <div class="d-flex flex-column">
                                                                ...
                                                                <div class="d-flex align-items-center py-3">
                                                                    ...
                                                                    <div class="px-3">
                                                                        <form
                                                                            action="{{ route('remove.from.cart', $item->id) }}"
                                                                            method="POST">
                                                                            @csrf
                                                                            <button type="submit"
                                                                                class="fs-6 text-danger bg-transparent border-0">Remove</button>
                                                                        </form>
                                                                    </div>
    ...
    <tr>
        <td class="thumbnail">
            <a href="#" style="width: 100px;">
                <img src="{{ asset('storage/' . $item->image) }}" alt="cart-product-1" width="100" height="100" loading="lazy" />
            </a>
        </td>
        <td class="name">
            <div class="d-flex flex-column">
                <p class="mb-0 text-dark product_cart_title">{{ $item->name }}</p>
                 <div class="d-flex align-items-center py-3">
                    {{-- <div class="product-item-quantity">
                        <div class="quantity-controls">
        ...
        <td class="price text-dark fw-500">
            {{ $currencySymbol }}<span class="item-total-price">{{ number_format($item->price ) }}</span>
        </td>
    </tr>
@endforeach
                @endif
    ...
    <div class="row justify-content-between mt-3">
        <div class="col-auto">
            ...
            @foreach ($cart as $item)
                @php
                    $offerPrice = \App\Models\ProductColor::where('product_id', $item->product_id)->min('offer_price');
                    ...
                    <table class="cart-totals-table table align-middle text-dark">
                        <tbody style="border-bottom: 2px solid #cccccc54">
                            <tr class="cart-subtotal">
                                <th>Price</th>
                                <td>{{ $currencySymbol }}<span class="amount">{{ number_format($totalAmount, 2) }}</span>
                                </td>
                            </tr>
                            <tr class="cart-subtotal">
                                <th>Shipping</th>
                        ...
                        <tbody style="border-bottom: 2px solid #cccccc54">
                            ...
                            <tr class="w-100">
                                <td class="w-100">
                                    <div class="">
                                        <form method="GET" class="justify-content-center flex-column align-items-center"
                                            action="{{ route('cart') }}">
                                            <div class="row">
                                                <div class="col-lg-9">
